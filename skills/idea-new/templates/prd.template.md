...
> This section is created automatically after `/skill:idea-new <ticket>` — capture each Q/A pair and update the status to READY before `analyst-check`.
...
## AIDD:RESEARCH_HINTS
> Mandatory for `/skill:researcher`: provide at least `Paths` or `Keywords`.
